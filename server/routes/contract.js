...
const insertUserHistoryBody = {
  ...
  properties: {
    ...
    totalRewardsConsumers: {
      type: "array",
      minItems: 1,
      maxItems: 100,
      items: { type: "string", pattern: "^[0-9]+$" },
    },
    totalRewardsContentOwners: {
      type: "array",
      minItems: 1,
      maxItems: 100,
      items: { type: "string", pattern: "^[0-9]+$" },
...
const contractRoutes = async (app) => {
  ...
  app.get(
    ...
    { schema: { params: addressParamSchema } },
    async (request, reply) => {
  ...
  app.get("/getTransactions", async (request, reply) => {
    ...
    try {
      ...
      if (userID && day && month && year && assetID) {
        ...
        return reply
          .code(400)
          .send({ error: "bad_request", message: "Missing userID query parameter" });
      }
  ...
  app.post(
    "/insertUserHistory",
    { schema: { body: insertUserHistoryBody } },
    async (request, reply) => {
      if (!requireContract(reply)) return;
      try {
        const { userIds, totalDurations, totalRewardsConsumers, totalRewardsContentOwners } =
          request.body;
